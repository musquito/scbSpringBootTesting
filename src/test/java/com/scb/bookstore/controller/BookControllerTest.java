...
public class BookControllerTest {
    ...
    public void findAllBooks() throws Exception {
        ...
        Book book = new Book(1, "JAVA", "Pansak kanhphukeaw", new BigDecimal(750.0) );
        bookList.add(book);
        Mockito.when(scbExternalBookRepository.findAllBooking()).thenReturn(bookList);
        mockMvc.perform(get("/books")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().string(containsString("Pansak kanhphukeaw")))
    ...
    public void userOrder() throws Exception {
        List<Book> bookList = new ArrayList<>();
        Book book = new Book(5, "JAVA", "Pansak Kangphukeaw", new BigDecimal(750.0));
        bookList.add(book);
        book = new Book(6, "Angular", "Pansak Kangphukeaw",  new BigDecimal(550.0));
        bookList.add(book);
        Mockito.when(scbExternalBookRepository.findAllBooking()).thenReturn(bookList);
        ...
        mockMvc.perform(post("/users/orders")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{ \"orders\": [5, 6] }"))
                .andExpect(content().string(containsString("1300.0")))
                .andExpect(status().isOk());
    }
